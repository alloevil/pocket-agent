...
def _step_app_and_credentials(config: dict):
    ...
    print()
    print(f"  {C.bold('4)')} 事件与回调 → 订阅方式 → 必须选 {C.key('「使用长连接接收事件」')}")
    print(f"     {C.red('⚠️  若选成「发送至开发者服务器地址(Webhook)」，长连接能连上但收不到消息！')}")
    print(f"  {C.bold('5)')} 事件与回调 → 添加事件 → 务必加上 {C.key('  '.join(_REQUIRED_EVENTS))}")
    print(f"     {C.red('⚠️  漏加这条事件 = 给机器人发消息完全没反应（程序也不会报错）')}")
    print(f"     {C.dim('（再添加回调 ' + '  '.join(_REQUIRED_CALLBACKS) + ' 用于审批按钮）')}")
    print(f"  {C.bold('6)')} 版本管理与发布 → 创建版本并{C.bold('发布')}（状态需为{C.key('「已发布」')}才生效）")
    print()
